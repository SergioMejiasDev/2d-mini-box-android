...
/// <summary>
/// Clase que controla las funciones de las bolas rojas.
/// </summary>
public class QBertRedBall : MonoBehaviour
{
    /// <summary>
    ...
    /// <summary>
    /// Verdadero si la clase está asignada a una bola morada. Falso si está asignada a una bola roja.
    /// </summary>
    [Header("PurpleBall")]
    [SerializeField] bool isPurple = false;
    /// <summary>
    /// La clase QBertPurbleBall asignada a la bola (en caso de ser morada).
    /// </summary>
    [SerializeField] QBertPurpleBall purpleClass = null;
    ...
    void FixedUpdate()
    {
        ...
        Vector2 newPosition = Vector2.MoveTowards(transform.position, destination, 0.025f);
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        // Si la bola morada llega a la parte inferior de la pirámide, se convierte en serpiente.

        if (isPurple)
        {
            if (collision.gameObject.name == "SnakeCollider")
            {
                transform.position = collision.gameObject.transform.position;
                purpleClass.enabled = true;
                enabled = false;
            }
        }
    }
